...
filename = '/home/cr2/cmtorres/ARs_DATA/vLHT/vLHT_1998.nc'

AR_shape = xr.open_dataset(filename)
AR_shape = AR_shape.sel(time=AR_shape.time.dt.hour == 12)  # Keep only 12 UTC
AR_shape['time'] = ('time', AR_shape['time'].dt.floor('D').data)  # Strip time to date only
AR_shape = AR_shape.sel(time=AR_shape['time'].dt.month.isin([11, 12, 1, 2, 3]))  # Filter SH summer months
AR_shape.coords['lon'] = (AR_shape.coords['lon'] + 180) % 360 - 180
AR_shape = AR_shape.sortby(AR_shape.lon)
AR_shape = AR_shape.sel(lon=slice(-150, -30), lat=slice(-30, -75))

HWs_ARS_KJS_just = pd.DataFrame({'date': pd.date_range(start='1998-11-16', end='1998-11-18', freq='1D')})

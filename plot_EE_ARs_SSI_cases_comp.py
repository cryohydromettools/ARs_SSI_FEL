...
time_range = HWs_ARS_KJS_just['date'].to_list()
print(time_range)
...
Z850_anom = Z850_an.sel(time=time_range).mean('time').z_anomaly[0]#*100
U850_anom = U850_an.sel(time=time_range).mean('time').u_anomaly[0]
...
levels_precip = np.arange(0, 6.1, 0.50)
...
levels_ws = np.arange(0.0, 10.1, 1)
...
# --- Plot precipitation (mm) ---
levels_precip = np.arange(100, 401, 25)

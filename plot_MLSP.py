...
for i in files:
	...
	data = data.where(mask<= 300, np.nan)
	...
	cmaps=plt.colormaps.get_cmap('seismic')
	val_levels= np.arange(950,1035,5)
	...
	cbar.set_label('hPa', fontsize=14, weight='bold')



	timeidx1 = str(data.valid_time.values)[:-16]

	ax_ctt.set_title('MSLP'+' on '+timeidx1, {"fontsize" : 14}, weight='bold')


	fig.savefig('MSLP_'+timeidx1+'.png', dpi = 500, facecolor='w', bbox_inches = 'tight', 
	            pad_inches = 0.1)
